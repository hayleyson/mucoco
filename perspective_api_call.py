...
def toxicity_score(generations_df, text_col, response_file, proba_file, perspective_rate_limit=5):
    ...
    for i, row in tqdm(generations_df.iterrows(), total=len(generations_df.index), desc='Evaluating toxicity via perspective API'):
        ...
        generations = [row[text_col]]  # - hayley
...
@click.option('--text-col', default='text', type=str, help='Name of text column in data')
@click.option('--nsamples', default=-1, type=int, help='Number of samples to evaluate')
...
def main(dpath, text_col, nsamples, outpath, rseed):
