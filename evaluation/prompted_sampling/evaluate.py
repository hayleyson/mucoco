...
def contents_preservation_metrics(sources_file,outputs_file,results_file,task):
    ...
    edit_distance = EditDistance(tokenizer)
    normalized_dist = edit_distance.compute(prompt = "", 
                          predictions = source_predictions_['prediction'].tolist(),
                          references = source_predictions_['source'].tolist(),
                          )
    unnormalized_dist = edit_distance.compute(prompt = "", 
                          predictions = source_predictions_['prediction'].tolist(),
                          references = source_predictions_['source'].tolist(),
                          normalize = False)
    ...
    return sbleu_score, sbert_score, sbert_preserved_prop, sbert_preserved_count, normalized_dist, unnormalized_dist
...
class EditDistance:
    ...
    Adapted from losses.content_preservation.EditDistanceLoss
    """
    ...
    def compute(self, 
        ...
        Optional args:
          - edit_level: "char", "word", or "token"(default)
