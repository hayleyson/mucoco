        ...
        if model_path not in name2model: #making sure we are not loading the model twice in case some constraints use the same model. 
            ...
            if "Custom" in model_types[i]:
                ...
                model, tokenizer = define_model(num_classes=num_classes, 
                                                                        ...
                                                                        task="nli")
                model = lossbuilder.ModelWrapper(model)
                name2tokenizer[model_path] = tokenizer
                name2model[model_path] = model
...
def cli_main_arg_file(argument_file_path):
