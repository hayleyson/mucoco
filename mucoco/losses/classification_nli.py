...
@register_loss("classification_nli")
class ClassificationLoss(BaseLoss):
    ...
    def compute_loss(self, batch, preds, **kwargs):
        ...
        sep = torch.empty((source.size(0), 1)).long().to(self.device).fill_(self.sep_token_id)
        # eos = self.eos
        #input_tokens = torch.cat([bos, prefix, pred_tokens, eos], dim=1)

        embed_lut = self.model.get_input_embeddings()
        if isinstance(embed_lut, nn.Sequential): # if there is a linear layer to match classifier embedding with language model embedding
            input_embeds = torch.cat([embed_lut(bos), embed_lut(source), embed_lut(sep), embed_lut[1](pred_embeds), embed_lut(eos)], dim=1) * kwargs["embed_scale"]
        else:
            input_embeds = torch.cat([embed_lut(bos), embed_lut(source), embed_lut(sep), pred_embeds, embed_lut(eos)], dim=1) * kwargs["embed_scale"]
    ...
    def compute_gold_loss(self, batch, **kwargs):
        ...
        with torch.no_grad():
            ...
            target = torch.cat([bos, source, sep, target, eos], dim=1)
