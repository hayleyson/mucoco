...
from new_module.utils.utils import ravel, unravel, unravel_toxicity_data, load_sc_energy_model
...
def _source_df_after_unravel_ravel(sources_df: pd.DataFrame) -> pd.DataFrame:
    ...
    `ravel` expects that string in column ``text`` (see ``new_module/utils/utils.py``).
    """
