...
def evaluate(run_path, generations_file_path, metrics, **kwargs):
    ...
    if run_path != "": ## if wandb run path is provided.
        ...
        output_file = "results_filled.txt"
    ...
    if "ppl-big" in metricset: #GPT2-XL
        ...
        if run_path != "":
            run.summary.update({'ppl_filled': ppl, 'total_ppl_filled': total_ppl})
    ...
    if 'toxicity' in metricset:
        ...
        if run_path != "":
            run.summary.update({'avg_max_toxicity_filled': avg_max_toxicity, 'toxic_probability_p_filled': toxic_probability_p,
                'avg_toxicity_filled': avg_toxicity, 'toxic_probability_s_filled': toxic_probability_s})
    ...
    if 'toxicity-int' in metricset:
        ...
        if kwargs['toxicity_model_path'] == "/shared/s3/lab07/hyeryung/loc_edit/roberta-base-jigsaw-toxicity-classifier-with-gpt2-large-embeds/step_2600_best_checkpoint/":
            kwargs['toxicity_model_path'] = "/shared/s3/lab07/hyeryung/loc_edit/roberta-base-jigsaw-toxicity-classifier-with-gpt2-large-embeds-2/step_2600_best_checkpoint/"
        ...
        if run_path != "":
            run.summary.update({'avg_max_toxicity_int_filled': avg_max_toxicity, 'toxic_probability_p_int_filled': toxic_probability_p,
                'avg_toxicity_int_filled': avg_toxicity, 'toxic_probability_s_int_filled': toxic_probability_s})
    ...
    if 'formality-ext' in metricset:
        ...
        if run_path != "":
            run.summary.update({'avg_formality_filled': avg_formality, 'formal_proba_filled': formal_proba})
    ...
    if 'formality-int' in metricset:
        ...
        if run_path != "":
            run.summary.update({'avg_formality_int_filled': avg_formality, 'formal_proba_int_filled': formal_proba})
    ...
    if 'sentiment-ext' in metricset:
        ...
        if run_path != "":
            ...
            run.summary.update({'positive_proba_filled': positive_proba, 
                                'positive_proba_p_std_filled': std_positive_proba_p,
                                'avg_positivity_filled': avg_positivity})
    ...
    if 'sentiment-int' in metricset:
        ...
        if run_path != "":
            ...
            run.summary.update({'positive_proba_int_filled': positive_proba, 
                                'positive_proba_p_std_int_filled': std_positive_proba_p,
                                'avg_positivity_int_filled': avg_positivity})
    ...
    if "dist-n" in metricset:
        ...
        if run_path != "":
            run.summary.update({'dist-1_filled': dist1, 'dist-2_filled': dist2, 'dist-3_filled': dist3})
    ...
    if "repetition" in metricset:
        ...
        if run_path != "":
            run.summary.update({'rep_rate_filled': rep_rate})
    ...
    if "fluency" in metricset:
        ...
        if run_path != "":
            run.summary.update({'fluent_proba_filled': fluency})
