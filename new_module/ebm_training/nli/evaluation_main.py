...
from new_module.ebm_training.nli.data_utils.datasets_loaders import NLI_Dataset, NLI_DataLoader
from new_module.ebm_training.nli.data_utils.prepare_datasets import load_nli_data
from new_module.ebm_training.nli.training.train_val_helpers import validate_model, plot_observed_predicted_boxplot, plot_observed_predicted_boxplot_v2   
from new_module.ebm_training.nli.losses.losses import CustomMarginRankingLoss, PairwiseLogisticLoss, MSE_MarginRankingLoss
...
wps90kq3""".split()
    ...
    results_df.to_excel(f"new_module/ebm_training/nli/evaluation_results/nli_energynet_metrics_{datetime.datetime.strftime(datetime.datetime.today(), '%Y%m%d')}_2.xlsx", index=False)