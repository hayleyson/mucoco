...
def train_energy(energynet, opt, pos_dataloader, neg_dataloader, learn_threshold = True, device = 'cpu', params = None):
...
def evaluate_energy(energynet, pos_dataloader, neg_dataloader, device = 'cpu', params = None):
    if params['eval']['pairwise'] == 'arbitrary_pairs':
        return evaluate_energy_arbitrary(energynet, pos_dataloader, neg_dataloader, device, params)
    elif params['eval']['pairwise'] == 'two_pairs':
        return evaluate_energy_two_pairs(energynet, pos_dataloader, neg_dataloader, device, params)
     
def evaluate_energy_fine_grained(energynet, pos_dataloader, neg_dataloader, side_pos=1, side_neg=-1, device = 'cpu', params = None):
    ...
    if params['energynet']['output_form'] == 'real_num':
        ...
        pos_result = [e for e in result['e_pos'] if side_pos*(e - energynet.threshold) < 0]
        ...
        auroc = 0.5
        precision, recall, f1, _ = precision_recall_fscore_support(result['gold'], result['pred'], average='macro')
...
def evaluate_energy_one_semantic(energynet, dataloader, side, device = 'cpu', params = None):
...
def evaluate_supervised_one_semantic(energynet, dataloader, side=None, device = 'cpu', params = None):
...
def evaluate_supervised_one_to_one(energynet, dataloader, side=None, device = 'cpu', params = None):
    ...
    acc = len([i for i in range(len(result['pred'])) if result['pred'][i] == result['gold'][i]]) / len(result['pred'])
...
def evaluate_energy_one_to_one(energynet, dataloader, side=None, device = 'cpu', params = None):
...
def evaluate_energy_many_to_one(energynet, dataloader, side=None, device = 'cpu', params = None):
...
def evaluate_supervised_many_to_one(energynet, dataloader, side=None, device = 'cpu', params = None):
...
def one_to_one_maximum_tolerance_ratio(energynet, seq_of_dataloaders, labels, device = 'cpu', params = None):
    ...
    best_tolerance_f1 = []
    for set_size in result['set_sizes'].keys():
        tolerance_ratio_result = tolerance_ratio_analysis(set_size_incon_ratios[set_size]['consistent'], set_size_incon_ratios[set_size]['inconsistent'])
    ...
    tolerance_ratio_result = tolerance_ratio_analysis(consistent_set_total, inconsistent_set_total)
...
def evaluate_energy_arbitrary(energynet, pos_dataloader, neg_dataloader, device = 'cpu', params = None):
    ...
    if params['energynet']['output_form'] == 'real_num':
        ...
        correct_by_trained_threshold = len([e for e in result['e_pos'] if e <= energynet.threshold] + 
...
def evaluate_energy_two_pairs(energynet, pos_dataloader, neg_dataloader, device = 'cpu', params = None):
    ...
    if params['energynet']['output_form'] == 'real_num':
        ...
        for pairs in tqdm(zip(pos_dataloader, neg_dataloader)):
            ...
            for pair in pos:
                # print("pair:", pair)
                pos_inputs = pair[0]
                pos_inputs_comb = comb_input(pos_inputs, cls_token = cls_token, sep_token = sep_token)
                # print("pos_inputs_comb[0]:", pos_inputs_comb[0])
                e_pos_out, _ = energynet.energy_model(pos_inputs_comb, pair_only = True)
                e_pos_incon_pred = [float(e) for e in e_pos_out if float(e) > energynet.threshold]
                e_pos_pred = 0 if len(e_pos_incon_pred) <= (len(pos_inputs)-1)/2 else 1
                result['pred'].extend([e_pos_pred])
                result['gold'].extend([0])

            for pair in neg:
                neg_inputs = pair[0]
                neg_inputs_comb = comb_input(neg_inputs, cls_token = cls_token, sep_token = sep_token)
                e_neg_out, _ = energynet.energy_model(neg_inputs_comb, pair_only = True)
                e_neg_incon_pred = [float(e) for e in e_neg_out if float(e) > energynet.threshold]
                e_neg_pred = 0 if len(e_neg_incon_pred) <= (len(neg_inputs)-1)/2 else 1
    ...
    for i, pairs in enumerate(dataloader):
        ...
        if i%100 == 0 or i == len(dataloader)-1:
            if label == None:
                ...
                accuracy = accuracy_score(pre_defined_gold[:len(pred)], pred, normalize = True)
...
def locate_energy(locate_net, dataloader, device = 'cpu', params = None):
    ...
    elif (params['locate']['type'] == 'gradnorm') or (params['locate']['type'] == 'attention'):

        # inference
        
        for pairs in tqdm(dataloader):
            
            # locate by gradnorm은 OOM 문제가 있어서,
            # 이 곳에서 single instance에 대한 metric 계산 및 consistent pair에 대한 logic 도 구현한다.
            
            inputs = [p[0] for p in pairs]
            golds = [p[1] for p in pairs]
            original_text.extend(inputs)
            
            # set 크기에 따른 skip (아예 data set 에서 제외)
            pair_num = int(pairs[0][0].count(locate_net.locate_method.sep_token))
            if pair_num >=4:
                data_len +=1
            else:
                continue
            ...
            output, hidden_states_or_attentions = locate_net.locate_method.energynet.energy_model(inputs, pair_only = True)
            threshold = locate_net.locate_method.energynet.threshold
            
            if (locate_net.locate_method.energynet.output_form == 'real_num'):
                e_val = float(output)
            elif (locate_net.locate_method.energynet.output_form == '2dim_vec'):
                e_val = float(output[:,1])
            else:
                raise NotImplementedError
            # print(f"initial energy value: {e_val}")
            
            # split into spans
            spans = detect_span(inputs[0], cls_token=locate_net.locate_method.energynet.energy_model.representation_model.tokenizer.cls_token, sep_token='.')
            pairs_dict = {s:i for i,s in enumerate(spans)}
            
            # if the set is inconsistent & the set contains more than one instance, do locate procedure
            if e_val > threshold:
                
                if params['locate']['select_method'] == 'recursive_max':
                    
                    # escape loop if the set is consistent or the set contains 1 item
                    while (e_val > threshold) and (len(spans) > 1):
                        
                        # locate
                        inputs = locate_net.locate_method.tokenizer.batch_encode_plus(inputs, padding=True, truncation = True, add_special_tokens = False)
                        locate_result = locate_net.locate_method.locate(inputs, output, hidden_states_or_attentions)
                    
                        # map index within set^(i-1) to index within set^(0)
                        for prediction_int in locate_result['prediction_list'][0]:
                            prediction_string = spans[prediction_int]
                            # print("prediction_int:", prediction_int)
                            # print("prediction_string:", prediction_string)
                            # print("pairs_dict[prediction_string]:", pairs_dict[prediction_string])
                            pred[0].append(pairs_dict[prediction_string])
                        
                        # update set: set^(i) = set^(i-1)\pred_list^(i)
                        # print("spans (before subtraction):", spans)
                        inputs = subtract_input(spans, locate_result['prediction_list'][0], cls_token=locate_net.locate_method.energynet.energy_model.representation_model.tokenizer.cls_token) 
                        spans = detect_span(inputs[0], cls_token=locate_net.locate_method.energynet.energy_model.representation_model.tokenizer.cls_token, sep_token='.')
                        # print("spans (after subtraction):", spans)
                        
                        # calculate E(set^(i)) to verify whether to end loop
                        output, hidden_states_or_attentions = locate_net.locate_method.energynet.energy_model(inputs, pair_only = True)
                ...
                elif params['locate']['select_method'] == 'recursive_above_avg':
                    
                    # initial call of above_avg : returns instance scores as well as prediction list
                    inputs = locate_net.locate_method.tokenizer.batch_encode_plus(inputs, padding=True, truncation = True, add_special_tokens = False)
                    # print(f"initial inputs: {spans}")
                    # inputs = locate_net.locate_method.tokenizer.batch_encode_plus(inputs, padding='max_length', add_special_tokens = False)
                    locate_result = locate_net.locate_method.locate(inputs, output, hidden_states_or_attentions)
                    
                    # remove items in prediction list from the original set: set^(0) \ pred_list
                    inputs = subtract_input(spans, locate_result['prediction_list'][0], cls_token=locate_net.locate_method.energynet.energy_model.representation_model.tokenizer.cls_token) 
                    ...
                    # print(f"energy value after subtracting pred_list: {e_val}")
                    # print(f"threshold: {threshold}")
                    
                    prediction_list = previous_prediction_list = locate_result['prediction_list']
                    locate_threshold = locate_result['thresholds']
                    instance_scores = locate_result['instance_scores_list']
                    avg = None
                    # print(f"prediction_list:", prediction_list)
                    # print(f"gold:", gold)
                    # print(f"instance_scores:", instance_scores)
                    # if the pred list includes ground truth (verified by checking if set \ pred list is consistent) and if the pred list contains more than 1 item
                    # we'd like to check if we can further reduce the pred list
                    i = 0
                    while (e_val < threshold) :
                        
                        
                        if avg: # add threshold from previous iteration
                            locate_threshold.append(avg)
                        # backup previous prediction list
                        previous_prediction_list = prediction_list 
                        if len(previous_prediction_list[0]) == 1:
                            break
                        # print(f"entering {i}th loop")
                            
                        # calculate average of span scores for itmes in the prediction list
                        instance_scores_of_prediction_list = [x for i, x in enumerate(instance_scores[0]) if i in prediction_list[0]]
                        avg = sum(instance_scores_of_prediction_list) / len(instance_scores_of_prediction_list)
                        # print(f"instance_scores_of_prediction_list: {instance_scores_of_prediction_list}")
                        # print(f"avg: {avg}")
                        
                        # find subset of the prediction list that has span score above the new average
                        prediction_list = [[prediction_list[0][i] for i, x in enumerate(instance_scores_of_prediction_list) if x >= avg]]
                        # print(f"new_prediction_list: {prediction_list}")
                        if prediction_list == previous_prediction_list: # if updated list is equal to previous list, means that we are stuck in a corner case where scores for all instances in the prediction list are the same
                            break

                        # remove the subset from the original set.
                        inputs = subtract_input(spans, prediction_list[0], cls_token=locate_net.locate_method.energynet.energy_model.representation_model.tokenizer.cls_token) 
                        # print(f"spans: {spans}")
                        ...
                        if (locate_net.locate_method.energynet.output_form == 'real_num'):
                            ...
                            raise NotImplementedError
                        # print(f"e_val: {e_val}")
                        # print(f"thresholds: {locate_threshold}")
                        i += 1
                    
                    # if not consistent, end the process and return the prediction list from previous (or initial) step.
                    # if i == 0:
                        # print(f"never entered loop. finished after initial prediction")
                    # else:
                        # print(f"exiting loop.")
                    pred = previous_prediction_list
                    # print(f"pred: {pred}")
                    ...
                    # output, hidden_states_or_attentions = locate_net.locate_method.energynet.energy_model.representation_model(inputs)
                    locate_result = locate_net.locate_method.locate(inputs, output, hidden_states_or_attentions)
                    
                    pred = locate_result['prediction_list']
                    locate_threshold = locate_result['thresholds']
                
            #### single instance에 대한 metric 계산 ####
            ...
            locate_thresholds.extend(locate_threshold)
    ...
    result['thresholds'] = locate_thresholds
        
    return result

def locate_energy_single_example(locate_net, input_text: list[str], params = None):
    
    locate_net.eval()
    
    if (params['locate']['type'] == 'gradnorm') or (params['locate']['type'] == 'attention'):
        ...
        # set 크기에 따른 skip (아예 data set 에서 제외) -> 데이터 불러올 때 처리하였으므로 삭제 
        # pair_num = int(input_text.count(locate_net.locate_method.sep_token))
        # if pair_num >=4:
        #     data_len +=1
        # else:
        #     return
    ...
    return pred, pred_token_indices
...
def subtract_input(spans, exclude_indices, cls_token):

    subtracted_list = []
    sub_string = f"{cls_token} "
    sub_string += ' '.join([spans[j] for j in range(len(spans)) if j not in exclude_indices])
...
def locate_baseline(model, dataloader, device = 'cpu', params = None):
    """GPT, LLaMA 등 baseline 모델들의 locate 성능 평가를 위한 함수입니다.
...
def inconsistency_detect(language_model, energy_model, folder_path,  device = 'cpu', params = None):
  
    result = {
        'llm_consistency': 0, 
        'llm_accuracy': 0,
        'energy_accuracy_gold_pairs': 0,
        'energy_accuracy_generated_pairs': 0,
    }
    pred = []
    gold = [] # 0: consistent, 1: inconsistent
    time_seconds = {}
    set_sizes = {}
    llm_accuracy_by_sizes = {}
    llm_consistency_by_sizes = {}
    energy_acc_gold_pairs_by_sizes = {}
    energy_acc_generated_pairs_by_sizes = {}
    ...
    for filename in files:
        ...
        language_model_result = language_model.wiqa_separate_inference(one_json)

        gold_qa_pairs = generate_qa_pairs(
            [one_json["qa_pairs"][idx]["question"] for idx in range(len(one_json["qa_pairs"]))],
            language_model_result['gold'])
        llm_generated_qa_pairs = generate_qa_pairs(
            [one_json["qa_pairs"][idx]["question"] for idx in range(len(one_json["qa_pairs"]))],
            language_model_result['pred'])

        e_vals_gold, _ = energy_model.energy_model(gold_qa_pairs)
        e_vals_generated, _ = energy_model.energy_model(llm_generated_qa_pairs)
    ...
    for i, pairs in enumerate(dataloader):
        ...
        # print("pairs:", pairs)
        start = time.time()
        evaluate_result = language_model.predict(pairs)
...
def fine_tune_energy(energynet_finetune, energynet_pre_trained, opt, pos_dataloader, neg_dataloader, learn_threshold = True, device = 'cpu', params = None):
    """
    dataloader에 original data와 fine-tune data가 동시에 섞여서 들어온 상황이라고 가정. (mixed_dataset)
...
def fine_tune_energy_with_l2(energynet_finetune, energynet_pre_trained, opt, pos_dataloader, neg_dataloader, learn_threshold = True, device = 'cpu', params = None):
...
def determine_best_split(array_positive, array_negative):
    ...
    best_precision_threshold_list, best_recall_threshold_list, best_f1_threshold_list, best_acc_threshold_list = [0], [0], [0], [0]
    best_precision, best_recall, best_f1, best_acc = 0, 0, 0, 0

    if (len(pos) == 0) and (len(neg) == 0):
        return {'precision': best_precision, 'precision_threshold': min(best_precision_threshold_list),
        'recall': best_recall, 'recall_threshold': max(best_recall_threshold_list),
        'f1': best_f1, 'f1_threshold': sum(best_f1_threshold_list) / len(best_f1_threshold_list),
        'acc': best_acc, 'acc_threshold': sum(best_acc_threshold_list) / len(best_acc_threshold_list),}
    ...
    return {
        'precision': best_precision, 'precision_threshold': min(best_precision_threshold_list),
        'recall': best_recall, 'recall_threshold': max(best_recall_threshold_list),
        'f1': best_f1, 'f1_threshold': sum(best_f1_threshold_list) / len(best_f1_threshold_list),
        'acc': best_acc, 'acc_threshold': sum(best_acc_threshold_list) / len(best_acc_threshold_list),
        }
