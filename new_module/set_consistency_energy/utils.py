...
def parser_add(parser, mode = 'train'):
    ...
    parser.add_argument('--finetune_job_id', type=int, default = 0) # 해당 jobid를 갖는 모델을 찾아서 job_id 폴더에 결과를 넣을 것임
    ...
    parser.add_argument('--locate_type', type=str)
    parser.add_argument('--agg_method', type=str, default = '')
    parser.add_argument('--select_method', type=str, default = 'above_avg')
    ...
    parser.add_argument('--attentions_num_layer', type=int)
    parser.add_argument('--dev', action='store_true', help='if true, use eval2 for evaluation. if false, use test set.')
...
def params_add(args, mode = 'train'):
    ...
    # Additional configs
    if params['energynet']['loss_type'] in {'supervised', }:
...
def draw_violin_plot(total_info_for_violinplot, vio_plot_column_name, eval_steps_names, save_paths):
    df = pd.DataFrame(data=total_info_for_violinplot, columns=vio_plot_column_name)

    colors = ['#80C5E7' if i in [0, 2, 5, 9] else '#E8B97A' for i in range(len(df['datatype'].unique()))]
    ax = sns.boxplot(data=df, y='datatype', x='energy value', patch_artist = True)

    for patch, color in zip(ax.patches, colors):
        patch.set_facecolor(color)

    # 원본 datatype 라벨과 변경할 LaTeX 라벨 매핑
    label_mapping = {
        'con': r"$S_{C}$",
        'incon': r"$S_{I}$",
        'con_con': r"$S_{CC}$",
    ...
    # 현재 y축의 위치와 기존 레이블 가져오기
    unique_steps = df['datatype'].unique()
    y_ticks = range(len(unique_steps))

    # 새로운 LaTeX 레이블로 변경
    plt.yticks(ticks=y_ticks, labels=[label_mapping[label] for label in unique_steps])
    ...
    fig = ax.get_figure()
    for p in save_paths:
        fig.savefig(p[:-4] + "_datatype_wo_threshold_" + p[-4:])
    plt.close()
    ...
    fig = ax.get_figure()
    for p in save_paths:
        fig.savefig(p[:-4] + "_datatype_with_threshold_" + p[-4:])
    plt.close()
...
def draw_violin_plot(total_info_for_violinplot, vio_plot_column_name, eval_steps_names, save_paths):
    ...
    desired_order = [
        'con', 'con_con', 'con_con_con', 'con_con_con_con',          # S_{C}, S_{CC}, S_{CCC}, S_{CCCC}
        'con_con_con_incon', 'con_con_incon', 'con_incon', 'incon',     # S_{CCCI}, S_{CCI}, S_{CI}, S_{I}
        'con_con_incon_incon', 'con_incon_incon', 'incon_incon',         # S_{CCII}, S_{CII}, S_{II}
        'con_incon_incon_incon', 'incon_incon_incon', 'incon_incon_incon_incon'  # S_{CIII}, S_{III}, S_{IIII}
    ]
    df['datatype'] = pd.Categorical(df['datatype'], categories=desired_order, ordered=True)
    ...
    bg_colors = {
        'con': '#80C5E7', 'con_con': '#80C5E7', 'con_con_con': '#80C5E7', 'con_con_con_con': '#80C5E7',
        'con_con_con_incon': '#E8B97A', 'con_con_incon': '#E8B97A', 'con_incon': '#E8B97A', 'incon': '#E8B97A',
        'con_con_incon_incon': '#E8B97A', 'con_incon_incon': '#E8B97A', 'incon_incon': '#E8B97A',
        'con_incon_incon_incon': '#E8B97A', 'incon_incon_incon': '#E8B97A', 'incon_incon_incon_incon': '#E8B97A'
    }
    ...
    sns.boxplot(data=df, y='datatype', x='energy value', order=desired_order,
                patch_artist=True, boxprops={'facecolor': 'white', 'edgecolor': 'black'}, ax=ax)

    # Set y-axis tick labels to the LaTeX labels
    plt.yticks(ticks=range(len(desired_order)), labels=[label_mapping[label] for label in desired_order], fontsize=12)
...
def draw_violin_plot_noneface(total_info_for_violinplot, vio_plot_column_name, eval_steps_names, save_paths):
    ...
    # Create the boxplot (without filling the boxes, i.e. transparent boxes with black edges)
    sns.boxplot(data=df, y='datatype', x='energy value', order=desired_order,
                patch_artist=True, boxprops={'facecolor': 'none', 'edgecolor': 'black'}, ax=ax)
...
def draw_violin_plot_whiteface_noarrow(total_info_for_violinplot, vio_plot_column_name, eval_steps_names, save_paths):
    ...
    plt.tight_layout()
    fig.savefig(save_paths[0][:-4] + "whiteface_noarrow_violinplot" + save_paths[0][-4:])
    plt.close()


def draw_violin_plot_noneface_noarrow(total_info_for_violinplot, vio_plot_column_name, eval_steps_names, save_paths):
...
def draw_hist(total_info_for_hist, hist_column_name, save_paths):
    """
    hist_column_name에 꼭 "datatype", "threshold", "datatype", "side" 이 있어야 함
    """
    df = pd.DataFrame(data = total_info_for_hist, columns = hist_column_name)

    ax = sns.histplot(data = df, y = 'datatype', x = 'energy value', binwidth = 0.01, hue = "side")
    unique_steps = df['datatype'].unique()
    for i in range(len(unique_steps) - 1):
...
def draw_violin_plot_supervised(total_info_for_violinplot, vio_plot_column_name, eval_steps_names, save_paths):
    df = pd.DataFrame(data = total_info_for_violinplot, columns = vio_plot_column_name)
    group_colors = {
        'con': '#fa1407', # red
