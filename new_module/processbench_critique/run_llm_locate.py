...
def apply_chat_template(toker, messages):
    ...
    return input_prompt  # Return the string prompt instead of token IDs
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default='new_module/processbench_critique/outputs')
    ...
    if 'Qwen3.5' in args.model_path:
        toker = AutoProcessor.from_pretrained(args.model_path)
    else:
        toker = AutoTokenizer.from_pretrained(args.model_path)
    TEMPLATE = open('new_module/processbench_critique/critique_template.txt').read().strip()
    ...
    for config in args.configs:
        ...
        if config == 'prm800k':
            input_data = read_prm800k_jsonl("new_module/data/prm800k/phase2_test.jsonl")
        else:
            input_data = load_dataset('Qwen/ProcessBench', split=config)    

        
        prompts = [apply_chat_template(toker, prepare_input_boxed(TEMPLATE, e))
                   for e in input_data]
