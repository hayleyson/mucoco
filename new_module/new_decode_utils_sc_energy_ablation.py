...
def get_beam_4sce(source_text:str, 
                    ...
                    config:dict,
                    batch_size:int=16,
                    consider_lefthand_instance_for_fluency=False,
                    consider_sc_energy=False) -> List[List[str]]:
    ...
    for curr_edit_index in edit_indices:
        ...
        if consider_lefthand_instance_for_fluency:
            tmp_hypotheses_dec1 = [lefthand_instance_text + hyp for hyp in tmp_hypotheses_dec]
        else:
...
def editing_4sce(source_text:str, test_sent_orig:str, test_sent:str, test_sent_span_lengths:List[int], 
                ...
                consider_lefthand_instance_for_fluency=False,
                consider_lefthand_instance_for_fluency_inner_beam=False,
                consider_sc_energy_inner_beam=False) -> \
                    Tuple[List[str],torch.FloatTensor,torch.BoolTensor,torch.FloatTensor]:
    """
    ...
    for i in range(len(mask_spans)):
        ...
        beam_outputs, _ = get_beam_4sce(source_text, 
                                ...
                                config,
                                batch_size=batch_size,
                                consider_lefthand_instance_for_fluency=consider_lefthand_instance_for_fluency_inner_beam,
                                consider_sc_energy=consider_sc_energy_inner_beam)
        hypotheses.extend(beam_outputs)
        ...
        hypotheses_with_leftside_contexts = []
        if i < len(mask_spans) -1 :
            if consider_lefthand_instance_for_fluency:
                hypotheses_with_leftside_contexts = [lefthand_instance_text + x + test_sent_merged[mask_spans[i][1]:mask_spans[i+1][0]] for x in hypotheses]
            hypotheses = [x + test_sent_merged[mask_spans[i][1]:mask_spans[i+1][0]] for x in hypotheses]
        else:
            if consider_lefthand_instance_for_fluency:
                hypotheses_with_leftside_contexts = [lefthand_instance_text + x + test_sent_merged[mask_spans[i][1]:] for x in hypotheses]
            hypotheses = [x + test_sent_merged[mask_spans[i][1]:] for x in hypotheses]
        # logger.debug(f"hypotheses: {hypotheses}")
        if consider_lefthand_instance_for_fluency:
            data_loader1 = DataLoader(CustomDataset(hypotheses_with_leftside_contexts),batch_size=batch_size)
        else:
