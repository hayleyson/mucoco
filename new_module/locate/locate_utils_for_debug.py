...
""")
    ...
    if "gpt2" in args.input_path:
        ...
        def collate_fn(batch):
            texts = tokenizer.batch_decode([torch.LongTensor(example["tokens"]) for example in batch])
            return texts
            # input_ids = pad_sequence([torch.LongTensor(example["tokens"]) for example in batch], padding_value=tokenizer.pad_token_id, batch_first=True) 
            # print(f"input_ids: {input_ids}")
            # batch = {"input_ids": input_ids,
    ...
    for batch in dataloader:
        # batch.to(device)
        # prediction, method, model, tokenizer, max_num_tokens = 6, unit="word", device="cuda", **kwargs
        # results, scores = locate_main(args.method, model, tokenizer, batch, max_num_tokens = 6, num_layer=10, unit="word", use_cuda=True)
        results, scores = locate_main(batch, args.method, model, tokenizer, max_num_tokens = 6, num_layer=10, unit="word", use_cuda=True, label_id=1)
