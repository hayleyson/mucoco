...
def main(args):
    ...
    if args.resume_step:
        accelerator.init_trackers(project_name=wandb_project, 
                                config=config,
                                init_kwargs={"wandb": {"entity": wandb_entity, "id": args.resume_wandb_id, "notes": ckpt_save_path, "resume": "must"}})
    else:
        accelerator.init_trackers(project_name=wandb_project, 
                                    config=config,
    ...
    if args.resume_step:
        resume_path = f"{ckpt_save_path}/laststep_{args.resume_step}"
        logger.debug(f"resume_path: {resume_path}")
        if model_type == "RobertaCustomForSequenceClassification": 
            model, tokenizer = define_model(mod_path= resume_path + '/pytorch_model.bin', num_classes=num_classes, device=device)
        elif model_type == "AutoModelForSequenceClassification":
            model = AutoModelForSequenceClassification.from_pretrained(resume_path, num_labels=num_classes)
            tokenizer = AutoTokenizer.from_pretrained(resume_path)
        state_dict = torch.load(f"{ckpt_save_path}/laststep_{args.resume_step}/optimizer_scheduler.pt")
    ...
    if args.task == 'formality':
        ...
        try:
            valid_dataset = valid_dataset.rename_column('toxicity', 'labels')
        except:
            pass
    ...
    if args.resume_step:
        logger.info("Loading optimizer and scheduler saved in directory")
    ...
    if args.resume_step:
        ...
        epoch_to_resume = int(step / len(train_loader))
        innerbatch_step_to_resume = step % len(train_loader)
        best_val_loss = state_dict['best_val_loss']
        best_val_step = state_dict['best_val_step']
    else:
        step = 0
        epoch_to_resume = 0
        innerbatch_step_to_resume = 0
        best_val_loss = float("inf")
        best_val_step = -1
    
    ## setting seed for reproducibility in data_loaders!
    set_seed(42)
    for epoch in tqdm(range(num_epochs)[epoch_to_resume:]):

        model.train()
        train_loss = 0
        for i, batch in tqdm(enumerate(train_loader), total=len(train_loader)):
            
            if i < innerbatch_step_to_resume:
                continue
            ...
            if step % num_validate_steps == 0:
                ...
                if avg_val_loss < best_val_loss:
                    ...
                    with open(f"{ckpt_save_path}/step_{step}/val_loss.txt", 'w') as f:
                    ...
                    for fname in os.listdir(test_output_dir):
                        ...
                        if fname.startswith('step'):
                            ...
                            fileData[fname] = open(f"{test_output_dir}/{fname}/val_loss.txt").read()
                        else:
                            pass
            ...
            tokenizer.save_pretrained(f"{ckpt_save_path}/laststep_{step}")
            model.module.save_pretrained(f"{ckpt_save_path}/laststep_{step}")
            torch.save({"optimizer": optimizer.state_dict(),
                        "scheduler": scheduler.state_dict(),
                        "best_val_loss": best_val_loss,
                        "best_val_step": best_val_step
                        }, f"{ckpt_save_path}/laststep_{step}/optimizer_scheduler.pt")
            if step > 1:
                last_folder_name = f"{ckpt_save_path}/laststep_{step-1}"
                logger.debug(one_folder_name)
                if accelerator.is_main_process:
                    os.system('rm -r ' + last_folder_name)


            if (time.time() - start_time) >= 60*60*11:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--resume_step', type=int, help='step to resume training from')
