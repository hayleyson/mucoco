...
display_name = f"{config['method']}-{config['locate_unit']}-nps{config['num_edit_token_per_step']}-k{config['k_per_location']}-beam{config['beam_size']}-{config['selection_criteria']}-{config['loss_weights'][0]}-{config['loss_weights'][1]}-wandb"
    ...
    for sample_idx in range(config['num_samples'])[:]:
        ...
        if allsat:
            ...
            for _iter in range(config['n_iter']):
                ...
                # hypotheses = constrained_beam_search(source_batch, masked_sequence, torch.LongTensor(sorted(indices[0])), primary_mask_token_id, predicted_token_ids, primary_tokenizer, mlm_tokenizer, primary_model, config, beam_size = config['beam_size'])
