...
def editing_4sce(source_text:str, test_sent_orig:str, test_sent:str, test_sent_span_lengths:List[int], 
    ...
    for i in range(len(mask_spans)):
        ...
        lefthand_instance_enc['input_ids'] = lefthand_instance_enc['input_ids'].expand(curr_queue_size,-1)
        lefthand_instance_enc['attention_mask'] = lefthand_instance_enc['attention_mask'].expand(curr_queue_size,-1)
        ...
        righthand_instance_enc['input_ids'] = righthand_instance_enc['input_ids'].expand(curr_queue_size,-1)
        righthand_instance_enc['attention_mask'] = righthand_instance_enc['attention_mask'].expand(curr_queue_size,-1)

        input_tokens = torch.cat([lefthand_instance_enc.input_ids, inputs.input_ids, righthand_instance_enc.input_ids], dim=1).to(config['device'])
        attention_masks = torch.cat([lefthand_instance_enc.attention_mask, inputs.attention_mask, righthand_instance_enc.attention_mask], dim=1).to(config['device'])
        ...
        logits = logits[:, lefthand_instance_enc.input_ids.shape[1]:input_tokens.shape[1]-righthand_instance_enc.input_ids.shape[1], :]
        logits[:, :, special_token_ids] = -float("inf")
