...
def get_beam_4sce(source_text:str, 
    ...
    for curr_edit_index in edit_indices:
        ...
        tmp_hypotheses_dec2 = [nonlocated_instances_text + hyp for hyp in tmp_hypotheses_dec]
        
        data_loader1 = DataLoader(CustomDataset(tmp_hypotheses_dec),batch_size=batch_size)
...
def editing_4sce(source_text:str, test_sent:str, test_sent_span_lengths:List[int], 
                located_instance_index: int, 
                mlm:AutoModelForMaskedLM, mlm_tokenizer:AutoTokenizer, 
                lossfns:List[lossbuilder.BaseLoss], config: dict, batch_size:int=16) -> \
                    Tuple[List[str],torch.FloatTensor,torch.BoolTensor,torch.FloatTensor]:
    """
    Performs editing on a specific located instance (e.g., a QA pair in Set-LConVQA or a 
    sentence in Set-SNLI).
    ...
    Since the input text is a collection of instances (e.g., QA pairs for Set-LConVQA or sentences for Set-SNLI), 
    ...
    if "<mask>" not in test_sent:
        ...
        for instance in instances:
            if "<mask>" in instance:
                ...
                break
    # save a string of concatenated nonlocated instances
    nonlocated_instances = [x for i, x in enumerate(instances) if i != located_instance_index]
    nonlocated_instances_text = join_instances(nonlocated_instances)
    ...
    for i in range(len(mask_spans)):
        ...
        # -------------------------------------------------------------------------
        # Conduct MLM inference for token candidate generation
        # -------------------------------------------------------------------------
        
        # fill maximum number of <mask> tokens at the current span
        curr_full_text_hyp = [base_hyp + "<mask>" * max_mask_cnt_per_span[i] + test_sent_merged[mask_spans[i][1]:] for base_hyp in queue]
        ...
        # Prepend nonlocated instances such that nonlocated instances are considered for candidate generation 
        nonlocated_enc = mlm_tokenizer(
            mlm_tokenizer.bos_token + nonlocated_instances_text, add_special_tokens=False, return_tensors="pt", padding=True, truncation=True
        ).to(config['device'])
        nonlocated_enc['input_ids'] = nonlocated_enc['input_ids'].expand(curr_queue_size,-1)
        nonlocated_enc['attention_mask'] = nonlocated_enc['attention_mask'].expand(curr_queue_size,-1)
        
        input_tokens = torch.cat([nonlocated_enc.input_ids, inputs.input_ids], dim=1).to(config['device'])
        attention_masks = torch.cat([nonlocated_enc.attention_mask, inputs.attention_mask], dim=1).to(config['device'])
        ...
        logits = logits[:, nonlocated_enc.input_ids.shape[1]:]
        logits[:, :, special_token_ids] = -float("inf")
        ...
        # Truncate the sequence to the last <mask> token
        masked_sequence = [masked_sequence[ix, :indices_in_mlm_tokens_1[max_mask_cnt_per_span[i]*(ix+1)-1]+1] for ix in range(masked_sequence.shape[0])]
        masked_sequence = torch.nn.utils.rnn.pad_sequence(masked_sequence, batch_first=True, padding_value=mlm_tokenizer.pad_token_id)        
        ...
        hypotheses_with_nonlocated = [nonlocated_instances_text + x for x in hypotheses] 
        data_loader2 = DataLoader(CustomDataset(hypotheses_with_nonlocated),batch_size=batch_size)
