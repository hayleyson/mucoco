...
def main(config):
    ...
    print(config)
    run = wandb.init(project="formality-decoding", config=config, entity="hayleyson")
    ...
    display_name += f"-{config['source_style']}-to-{config['target_style']}-debugmode"
    ...
    outdir = os.path.join(config['output_dir_prefix'], 
                          display_name)
    ...
    for text_id in range(len(source_dataset)):
        ...
        for sample_idx in range(config['num_samples'])[:]:
            ...
            gold_losses = []
            # label_ids = [0, 0] # target label's ids for each loss
            label_ids = config['target_label_ids']
                ...
                if sample_idx == 0:
                    output = {
                        ...
                        "generations":[{
                            ...
                            "indices": indices, 
                            "orig_tokens_at_indices": name2tokenizer[config['model_paths'][0]].decode(predicted_batch[:, indices].squeeze()),
                            "allsat": best_allsat,
                            "losses": best_losses,
...
if __name__ == "__main__":
    ...
    # with open('data/formality/GYAFC_Corpus/Entertainment_Music/test/informal.ref0', 'r') as f:
    #     target_data.append(f.readlines())
    # with open('data/formality/GYAFC_Corpus/Entertainment_Music/test/informal.ref1', 'r') as f:
    #     target_data.append(f.readlines())
    # with open('data/formality/GYAFC_Corpus/Entertainment_Music/test/informal.ref2', 'r') as f:
    #     target_data.append(f.readlines())
    # with open('data/formality/GYAFC_Corpus/Entertainment_Music/test/informal.ref3', 'r') as f:
    #     target_data.append(f.readlines())
    
    # model = AutoModelForSequenceClassification.from_pretrained('models/mix_match_chkpts/form_em.bckp/models/checkpoint-250')
    # tokenizer = AutoTokenizer.from_pretrained('models/mix_match_chkpts/form_em.bckp/models/checkpoint-250', use_fast=False)
    
    # test_sent = 'Her name is Jessica.'
