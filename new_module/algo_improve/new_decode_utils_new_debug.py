...
def get_beam_hypotheses_v0_variable_length_v2(source_text:str, 
    ...
    for curr_edit_index in edit_indices:
        ...
        hypotheses_dec = [mlm_tokenizer.batch_decode(x, skip_special_tokens=True) for x in hypotheses]
        hypotheses_dec = sum(hypotheses_dec, [])
        print(f"-- partial hypotheses after unraveling disregarding the grouping by initial hypothesis: {hypotheses_dec}")
...
def editing_with_delete_variable_replace(source_text:str, test_sent:str, test_sent_span_lengths:List[int], 
    ...
    print(f"max_mask_cnt_per_span: {max_mask_cnt_per_span}")
    ...
    for i in range(len(mask_spans)):
        ...
        print('---')
        for ix in range(curr_queue_size):
            print(f'---{ix}th beam---')
            for jx in range(j):
                print(f"predicted_token_ids at {jx}th mask: {mlm_tokenizer.batch_decode(predicted_token_ids.indices[ix*j+jx], skip_special_tokens=True)}")
        print('---')
        ...
        if i == len(mask_spans) -1:
            ...
            print(f"-- selected 1 hypothesis for current ({i}th) span: {final_hypotheses}")
        else:
