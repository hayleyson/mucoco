...
from new_module.new_decode_utils_old import get_beam_hypotheses_v0, get_beam_hypotheses_v1, get_combi_hypotheses, final_reranking, analyze_span_lengths_and_count, editing_with_delete_variable_replace
...
def get_beam_hypotheses_v0_variable_length_considering_post_context_v2_only_primary(source_text:str, 
                    masked_sequence:torch.Tensor, 
                    post_context:str,
                    indices_in_mlm_tokens:Tuple[torch.Tensor],
                    predicted_token_ids:torch.Tensor,
    ...
    curr_loss = torch.zeros(len(tmp_hypotheses)).to(config['device'])
    for lossid, lossname in enumerate(config["losses"][:1]):
        with torch.no_grad():
            lossvalue = lossfns[lossid].compute_gold_loss(
                source_text, [x+post_context for x in mlm_tokenizer.batch_decode(tmp_hypotheses,skip_special_tokens=True)],
                label_id=config['target_label_ids'][lossid],
            ...
            for i in range(len(mask_spans)):
                ...
                if method == "1":
                    ...
                    hypotheses, scores = get_beam_hypotheses_v0_variable_length_considering_post_context_v2(source_text, 
                                            ...
                                            tmp_config)
                    hypotheses = hypotheses[0]
                    print(f"{i},beam,{time.time()-before_beam}")
                    # print(f"hypotheses: {hypotheses}")
                    ...
                    # hypotheses.extend(get_beam_hypotheses_v0_variable_length_considering_post_context(source_text, 
                    before_beam = time.time()
                    hypotheses, scores = get_beam_hypotheses_v0_variable_length_considering_post_context_v2_only_primary(source_text, 
                                            masked_sequence, 
                                            post_context,
                                            (indices_in_mlm_tokens_0, indices_in_mlm_tokens_1),
                                            predicted_token_ids.indices,
                    ...
                    # print(f"hypotheses: {hypotheses}")
                    if i == len(mask_spans) -1:
                        before_final_reranking = time.time()
                        # Scoring the hypotheses and select top beam hypotheses
                        batch_size = 64
                        ...
                        print(f"{i},rerank,{time.time()-before_final_reranking}")
