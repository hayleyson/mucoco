...
from laser_edit.edit.llm.nli_toxicity.prompts import get_prompt
from laser_edit.set_consistency_energy.baselines.LLM.lm_loader import lm_loader
from laser_edit.utils.sc_energy_utils import parse_set_text, convert_format, format_set_text
...
def generate_and_save_result(args):
    ...
    #     orig_text_path = "/home/saeheeeom/data/set_consistency/mucoco/laser_edit/base_lm_generate/baselm_gens/gpt-3.5-turbo-0125/nontoxic/gpt-3.5-turbo-0125_realtoxicityprompts_0shot_150.jsonl"
    # else:
    #     orig_text_path = '/data3/saeheeeom/set_consistency/mucoco/laser_edit/base_lm_generate/baselm_gens/gpt-3.5-turbo-0125/nontoxic/gpt-3.5-turbo-0125_realtoxicityprompts_noprompt_150.jsonl'
...
def generate_and_save_result_vllm_sc_energy(args, test_dataset, located_dataset):
    ...
    for batch, batch_orig in zip(located_data_loader[0], orig_data_loader[0]):
        if not locate_edit_idx[line_idx][0]:
            ...
            continue
        prompt = ""
        gen = format_set_text(batch[0][0], target_mode="llm", dataset=args.dataset)
        orig_gen = format_set_text(batch_orig[0][0], target_mode="llm", dataset=args.dataset)
    ...
    for count, req_out in enumerate(all_outputs):
        ...
        edited_set_text = parse_set_text(text, source_mode="llm", dataset=args.dataset)
