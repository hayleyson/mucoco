...
def main():
    ...
    parser_main.add_argument(
        "--located_results_file",
        type=str,
        default=None,
        help="Path to a processed LLM-located JSONL file with masked generations. When set, skips energy locating and requires --total_iteration 1.",
    )
    parser_main.add_argument(
        "--use_vllm",
        action="store_true",
    ...
    located_results_file = args_main.located_results_file
    if located_results_file is not None:
        if total_iteration > 1:
            parser_main.error("--located_results_file can only be used with --total_iteration 1")
        if not os.path.isfile(located_results_file):
            parser_main.error(f"--located_results_file does not exist or is not a file: {located_results_file}")
        located_results_file = os.path.abspath(located_results_file)
    ...
    if located_results_file is None:
        ...
        mlm_tokenizer = None
        print(f"Using precomputed located results from: {located_results_file}")
    ...
    for iter_idx in range(total_iteration):
        ...
        if located_results_file is not None:
            current_locate_output_file_path = located_results_file
            print("Using precomputed located results:", current_locate_output_file_path)
        else:
            current_locate_output_file_path = locate_output_file_path + f"_filtered_{iter_idx}"
            locate_texts_multi(
                energy_locators,
                locate_modes,
                energy_label_ids,
                mlm_tokenizer,
                input_file_path,
                current_locate_output_file_path,
                locate_edit_idx,
                locate_run_config,
            )
        ...
        'input_file_path': current_locate_output_file_path,
