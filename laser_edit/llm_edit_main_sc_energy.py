...
from laser_edit.utils.sc_energy_utils import load_sc_energy_model, parse_set_text, load_sc_energy_dataset
from laser_edit.locate.ebm.locate_utils import LocateMachine4SCE, LocateMachine
from laser_edit.locate.ebm.locate_utils import call_locate, union_masks
from laser_edit.set_consistency_energy.energynets.decomposition.no_decomposition import no_decomposition_loader
    ...
    with open(output_file, "w", encoding="utf-8") as outfile:
        for sample_idx, batch in enumerate(data_loader):
            ...
            if locate_edit_idx[sample_idx][0]:
                ...
                masked_set_texts.append(parse_set_text(merged_text, source_mode="ebm", dataset=locate_modes_list[0]))
...
if __name__ == "__main__":
    ...
    parser_main.add_argument("--input_file_path", type=str, required=True, help="Input JSONL path.")
    ...
    parser_main.add_argument("--llm_name", type=str, required=True, help="Name of the Hugging Face model.")
    parser_main.add_argument(
        "--use_incon_samples",
        action="store_true",
    ...
    llm_name = args_main.llm_name
    llm_name_for_file = os.path.basename(llm_name.rstrip("/")).lower()
    ...
    locate_output_file_path = directory + f'/located/{llm_name_for_file}_{exp_label}_located_{job_id}.jsonl'
    edit_output_file_path = directory + f'/edited/{llm_name_for_file}_{exp_label}_edited_{job_id}.jsonl'
    eval_output_file_path = directory + f'/losses/{llm_name_for_file}_{exp_label}_losses_{job_id}.txt'
    final_output_file_path = directory + f'/final/{llm_name_for_file}_{exp_label}_loc_edit_{job_id}.jsonl'
    ...
    # Load dataset
    test_datasets, test_steps_names = load_sc_energy_dataset(task, split="test", use_only_incon=True, n_samples=n_samples_cfg, random_seed=42)
    ...
    for iter_idx in range(total_iteration):
        ...
        'max_tokens': 150,
        'enable_thinking': True if 'qwen3' in llm_name.lower() else False,
        'dataset': task
    }
        ...
        eval_premises = ["" for _ in range(len(l_e_texts))]
