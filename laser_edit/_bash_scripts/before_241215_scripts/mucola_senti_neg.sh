...
#SBATCH --nodelist=n02
#SBATCH --gres=gpu:1
#SBATCH --job-name=mucola_senti_neg
#SBATCH --output='laser_edit/_slurm_outs/mucola_senti_neg_%j.out'
...
--dampness=0.1  \
--data=laser_edit/data/sentiment/dev_set.jsonl  \
--datastyle=jsonl  \
...
--metric=l2  \
--thresholds=-3  \
--model=gpt2-large:$DATA_DIR/loc_edit/models/roberta-base-yelp-sentiment-classifier-with-gpt2-large-embeds/step_114500_best_checkpoint  \
--model_dtype=fp32  \
...
--optim=embedgd_le  \
--optim-steps=200  \
--output_dir_prefix=outputs/sentiment/mucola/  \
--output-style=jsonl  \
--prefix-length=0  \
--random-example=true  \
...
--suffix-length=0  \
--target-type=embeds  \
--tokenizer=gpt2-large:$DATA_DIR/loc_edit/models/roberta-base-yelp-sentiment-classifier-with-gpt2-large-embeds/step_114500_best_checkpoint  \
--topic-target=none  \
