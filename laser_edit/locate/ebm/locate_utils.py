...
class LocateMachine4SCE:
    ...
    def _locate_instance(self, token_scores: torch.Tensor, instance_locations: List, batch_size: int) -> List:
        ...
        if self.params['locate']['instance']['agg_method'] == 'max':
            ...
            raise ValueError(f"Invalid aggregation method: {self.params['locate']['instance']['agg_method']}")
        ...
        if self.params['locate']['instance']['select_method'] in ['max', 'recursive_max']:
            ...
            raise ValueError(f"Invalid selection method: {self.params['locate']['instance']['select_method']}")
...
def call_locate(task: str, label_id: int, locator: Union[LocateMachine|LocateMachine4SCE], source_text: str, running_text: List[str], config: dict) -> List[str]:
    
    if (task in ["set_lconvqa", "set_snli", "set_nli"]):
...
if __name__ == "__main__":
    ...
    execution_time_file = file_name + ".time"
