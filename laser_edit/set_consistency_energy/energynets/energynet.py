...
class energynet(nn.Module):
    def __init__(self, params):
        ...
        # Energy Net을 위한 loss를 구하는 방법. Default = triplet_loss
        self.loss_function = None # determined by self.initialize()
        self.loss_type = params['energynet']['loss_type'] # e.x., triplet_loss, regression-based loss
        
        # 여러 x,y pair를 어떻게 나누어 feature extract 할 것인가에 관련. Default = no decomposition
        self.energy_model = None # determined by self.initialize()
        self.decomposition_type = params['energynet']['decomposition'] # e.x., (no / x-y / pairwise) decomposition
        
        # representation model. Default = RoBERTa
        self.representation_model = None # determined by self.initialize()
        self.representation_model_type = params['energynet']['repre_model'] # e.x., Longformer, RoBERTa
    ...
    def forward(self, input, pair_only= False):
        ...
        if self.loss_type == 'supervised':
            ...
            additional_info['pred'] = [int(e[-1] >= self.threshold) for e in additionals['probs']]
