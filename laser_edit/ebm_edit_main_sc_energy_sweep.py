...
from laser_edit.set_consistency_energy.energynets.energynet import energynet
from laser_edit.edit.ebm.decoding_utils import analyze_span_lengths_and_count, editing_4sce, compute_allsat_from_thresholds
...
root_dir = 'laser_edit/set_consistency_energy'
...
def main(config):
    ...
    if not config["debug"]:
        ...
        # Override config with swept hyperparameters
        config["max_tokens_per_span"] = wandb.config.max_tokens_per_span
        config["num_edit_tokens_per_step"] = wandb.config.num_edit_tokens_per_step
        config["loss_weights"] = [wandb.config.loss_weight_0, wandb.config.loss_weight_1]
    ...
    loss_args = LossArgs(task=config['task'],
                        device=device)
    ...
    locator = LocateMachine4SCE(model_config, energy_net, config['task'])
    ...
    del lossfns, mlm, causal_lm, energy_net
    torch.cuda.empty_cache()

    run_generation_evaluation(
            run_path,
            outfile,
            "set-consistency,fluency,contents-preservation,h1",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", dest="config_path_flag", type=str)
    ...
    # W&B agents append swept parameters as CLI flags; parse them so argparse
    # does not reject the run before wandb.init can expose them via wandb.config.
    parser.add_argument("--loss_weight_0", type=float)
    parser.add_argument("--loss_weight_1", type=float)
    parser.add_argument("--k_per_location", type=int)
    parser.add_argument("--beam_size", type=int)
    parser.add_argument("--num_edit_tokens_per_step", type=int)
    parser.add_argument("--max_tokens_per_span", type=int)
    ...
    if config_path is None:
        parser.error("config path is required, either as CONFIG_PATH or --config CONFIG_PATH")

    with open(config_path, 'r') as f:
        basic_config = yaml.load(f, Loader=yaml.FullLoader)

    basic_config.update({'device': device})
    cli_overrides = {
        key: value
        ...
        if key in {
            "k_per_location",
            "beam_size",
            "num_edit_tokens_per_step",
            "max_tokens_per_span",
        } and value is not None
    }
    if args.loss_weight_0 is not None or args.loss_weight_1 is not None:
        loss_weights = list(basic_config["loss_weights"])
        if args.loss_weight_0 is not None:
            loss_weights[0] = args.loss_weight_0
        if args.loss_weight_1 is not None:
            loss_weights[1] = args.loss_weight_1
        cli_overrides["loss_weights"] = loss_weights
    basic_config.update(cli_overrides)
