...
def load_nli_data(dev_split_size=0.1, force_reload=False,
                  output_file_path = 'laser_edit/ebm_training/data/nli/snli_mnli_anli_train_dev_with_finegrained.jsonl'):
    ...
    if os.path.exists(output_file_path) and not force_reload:
        ...
        # 1. Load data
        mnli1 = pd.read_json('laser_edit/ebm_training/data/nli/multinli_1.0/multinli_1.0_dev_matched.jsonl', lines=True)
        mnli2 = pd.read_json('laser_edit/ebm_training/data/nli/multinli_1.0/multinli_1.0_dev_mismatched.jsonl', lines=True)
        snli = pd.read_json('laser_edit/ebm_training/data/nli/snli_1.0/snli_1.0_dev.jsonl', lines=True).rename(columns={'captionID': 'promptID'})
        anli_R1_train = pd.read_json('laser_edit/ebm_training/data/nli/anli_v1.0/R1/train.jsonl', lines=True)
        anli_R1_dev = pd.read_json('laser_edit/ebm_training/data/nli/anli_v1.0/R1/dev.jsonl', lines=True)
        anli_R2_train = pd.read_json('laser_edit/ebm_training/data/nli/anli_v1.0/R2/train.jsonl', lines=True)
        anli_R2_dev = pd.read_json('laser_edit/ebm_training/data/nli/anli_v1.0/R2/dev.jsonl', lines=True)
        anli_R3_train = pd.read_json('laser_edit/ebm_training/data/nli/anli_v1.0/R3/train.jsonl', lines=True)
        anli_R3_dev = pd.read_json('laser_edit/ebm_training/data/nli/anli_v1.0/R3/dev.jsonl', lines=True)
        ...
        smnli['3class_finegrained_labels'] = smnli['annotator_labels'].apply(lambda labels: [np.mean([1 if label == 'entail' else 0 for label in labels]), 
                                                                                             np.mean([1 if label == 'neutral' else 0 for label in labels]),
                                                                                             np.mean([1 if label == 'contradiction' else 0 for label in labels])])
        ...
        smnli = smnli[['pairID', 'premise', 'hypothesis', 'original_labels', 'label', '3class_finegrained_labels', 'finegrained_labels', 'genre', 'source', 'split']]
        ...
        anli_verifier_labels = pd.read_json('laser_edit/ebm_training/data/nli/anli_v1.0/verifier_labels_R1-3.jsonl', lines=True)
        ...
        anli['3class_finegrained_labels'] = anli['verifier labels'].apply(lambda labels: [np.mean([1 if label == 'e' else 0 for label in labels]),
                                                                                          np.mean([1 if label == 'n' else 0 for label in labels]),
                                                                                          np.mean([1 if label == 'c' else 0 for label in labels]),])
        ...
        anli = anli[['pairID', 'premise', 'hypothesis', 'original_labels', 'label', '3class_finegrained_labels', 'finegrained_labels', 'genre', 'source', 'split' ]]
...
def load_additional_nli_training_data(force_reload=False,
                                      output_file_path = 'laser_edit/ebm_training/data/nli/snli_mnli_anli_train_without_finegrained.jsonl'):
    if os.path.exists(output_file_path) and not force_reload:
        print('Preprocessed dataset already exists. Loading data from file')
        data = pd.read_json(output_file_path, lines=True)
        return data
        ...
        # 1. Load data
        mnli = pd.read_json('laser_edit/ebm_training/data/nli/multinli_1.0/multinli_1.0_train.jsonl', lines=True)
        snli = pd.read_json('laser_edit/ebm_training/data/nli/snli_1.0/snli_1.0_train.jsonl', lines=True).rename(columns={'captionID': 'promptID'})
        anli_R1_train = pd.read_json('laser_edit/ebm_training/data/nli/anli_v1.0/R1/train.jsonl', lines=True)
        anli_R2_train = pd.read_json('laser_edit/ebm_training/data/nli/anli_v1.0/R2/train.jsonl', lines=True)
        anli_R3_train = pd.read_json('laser_edit/ebm_training/data/nli/anli_v1.0/R3/train.jsonl', lines=True)
        ...
        smnli['3class_finegrained_labels'] = [None for _ in range(len(smnli))]
        ...
        anli['3class_finegrained_labels'] = [None for _ in range(len(anli))]
...
def load_nli_test_data(force_reload=False,
                        output_file_path = 'laser_edit/ebm_training/data/nli/snli_mnli_anli_test_with_finegrained.jsonl'):
    ...
    if os.path.exists(output_file_path) and not force_reload:
        ...
        # 1. Load data
        # no mnli test data
        snli = pd.read_json('laser_edit/ebm_training/data/nli/snli_1.0/snli_1.0_test.jsonl', lines=True).rename(columns={'captionID': 'promptID'})
        anli_R1_test = pd.read_json('laser_edit/ebm_training/data/nli/anli_v1.0/R1/test.jsonl', lines=True)
        anli_R2_test = pd.read_json('laser_edit/ebm_training/data/nli/anli_v1.0/R2/test.jsonl', lines=True)
        anli_R3_test = pd.read_json('laser_edit/ebm_training/data/nli/anli_v1.0/R3/test.jsonl', lines=True)
        ...
        smnli['original_labels'] = smnli['label'].replace({'entailment': 0, 'neutral': 1, 'contradiction': 2})
        smnli['label'] = smnli['label'].apply(lambda x: 0 if x == 'contradiction' else 1)
        smnli = smnli[['pairID', 'premise', 'hypothesis', 'original_labels', 'label', '3class_finegrained_labels', 'finegrained_labels', 'genre', 'source']]
        ...
        anli = anli.rename(columns={'uid': 'pairID'})
        anli = anli[['pairID', 'premise', 'hypothesis', 'original_labels', 'label', '3class_finegrained_labels', 'finegrained_labels', 'genre', 'source']]
