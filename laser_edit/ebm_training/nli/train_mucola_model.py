...
from laser_edit.ebm_training.nli.data_utils.datasets_loaders import NLI_Dataset, NLI_DataLoader, NLI_TrainBatchSampler_Binary, NLI_TrainBatchSampler_Continuous
from laser_edit.ebm_training.nli.data_utils.prepare_datasets import load_nli_data, load_additional_nli_training_data
from laser_edit.ebm_training.nli.training_utils.train_val_helpers import *
from laser_edit.ebm_training.nli.losses.losses import create_pairs_for_ranking, CustomMarginRankingLoss, PairwiseLogisticLoss, MSE_MarginRankingLoss

def main():
    
    config = load_config('laser_edit/ebm_training/nli/config_mucola_model.yaml')
    ...
    if config['energynet'].get('mucola_style', False):
        ...
        model, tokenizer = define_model(num_classes=num_classes, 
                                        ...
                                        embedding_model=config['energynet']['embedding_model'],
                                        task="nli")
    else:
    ...
    if config['energynet']['add_train_data']:
        train_add_data = load_additional_nli_training_data(output_file_path='laser_edit/ebm_training/data/nli/snli_mnli_anli_train_without_finegrained.jsonl')
