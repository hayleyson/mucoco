...
parser.add_argument('--generations_cleaned', type=bool, required=False, default=False)
...
if args.generations_cleaned:
    gens = []
    for i, row in enumerate(data):
        prompt = prompts.iloc[i, 0]
        if prompt in row:
            row = row.replace(prompt, '')
        gens.append(row.rstrip())
