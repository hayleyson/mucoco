...
# EPSILONS=(-5 -2.2 -1.4)
EPSILONS=(-5 -3) # the constraint is  log(negative_prob) - log (positive_prob) < epsilon, where positive_prob is the desired label
model_types=("energy" "mucola")
data_sources=("jigsaw" "gpt2")
...
do
    ...
    do
        ...
        do 
            ...
            NUM_LOG_STEPS=$NUM_PROJECT_STEPS
            OUTPUTDIR=outputs/toxicity/locate-edit-${data_source}-loc-${NUM_EDIT_TOKEN_PER_STEP}toks-${NUM_LOCATE_STEPS}steps-project-${NUM_PROJECT_STEPS}steps-${selection_criterion}-modeltype-${model_type}-locunit-${locate_unit}
            mkdir -p $OUTPUTDIR
